# Directive Engine Run

- Run ID: `d24a7bbd-2ea7-4014-a8cc-d1fdb59e7850`
- Received At: `2026-03-24T00:00:00.000Z`
- Candidate ID: `dw-live-scientify-engine-pressure-2026-03-24`
- Candidate Name: Scientify Mixed Adoption Target Pressure
- Source Type: `github-repo`
- Source Ref: `https://github.com/user/scientify`
- Selected Lane: `forge`
- Usefulness Level: `direct`
- Decision State: `route_to_forge_follow_up`
- Integration Mode: `reimplement`
- Proof Kind: `forge_runtime_proof`
- Run Record Path: `runtime/standalone-host/engine-runs/2026-03-24T00-00-00-000Z-dw-live-scientify-engine-pressure-2026-03-24-d24a7bbd.json`

## Mission Fit

Mixed runtime operationalization and engine self-improvement. Assess whether literature monitoring, orchestrated research pipeline, scheduled delivery, and project knowledge-state patterns should become a reusable runtime workflow in Forge, an Engine workflow or handoff improvement in Architecture, or stay in Discovery until the primary adoption target is clearer.

## Usefulness Rationale

Direct usefulness: the candidate targets reusable runtime capability, so the value is primarily useful as something the host can call or run again.

## Report Summary

Sync the route_to_forge_follow_up decision and reimplement integration plan into Directive Workspace reporting surfaces. Usefulness rationale: Direct usefulness: the candidate targets reusable runtime capability, so the value is primarily useful as something the host can call or run again.

## Routing Rationale

- No unresolved gap matched strongly enough, so the assessment relied on mission-fit and lane-signal scoring.
- Recommended forge because its lane score (21) exceeded the alternatives.
- Meta-usefulness signal is present (3/5), which strengthens Engine-improvement handling inside Architecture or Discovery.
- Queue-only is recommended because the candidate still needs more routing clarity before record expansion.

## Next Action

Open a bounded Forge follow-up and only involve host code through the engine adapter boundary.
