...
- Opened by: `codex-runtime-route-proof-followthrough`
...
- Decision reason: this mini-swe route-proof case is now the top repo-native Runtime host-selection blocker, and current repo truth already proves the repo-native DW web-host seam-review and manual-promotion pattern on adjacent OpenMOSS, Puppeteer, and Scientify cases. The smallest truthful next step is to make that product-owned host target explicit here without opening promotion, host integration, runtime execution, or automation.
...
- Existing repo-native host proof reused:
  ...
  - `shared/contracts/scientify-pressure-dw-web-host-runtime-promotion-guard.md`
...
- Explicit bounded pre-promotion profile/checker family for that slice:
  ...
  - `Focused compile-contract checker = npm run check:directive-real-mini-swe-agent-runtime-route-dw-web-host-seam-review-compile-contract`
...
- Explicit bounded pre-promotion host constraints for that slice:
  ...
  - `Safe output scope = mini-swe route seam-review page plus thin-host detail payload only; no promotion-record creation, no registry acceptance, no host integration writes, no runtime execution`
...
- Explicit bounded pre-promotion profile/checker family for that slice:
  ...
  - `Proof shape = dw_web_host_seam_review_snapshot/v1`
  - `Primary host checker = npm run check:directive-dw-web-host-runtime-seam-review`
  - `Focused input-package checker = npm run check:directive-real-mini-swe-agent-runtime-route-v0-dw-web-host-promotion-input-package`
  - `Contract path = shared/contracts/dw-web-host-seam-review-guard.md`
- Explicit bounded pre-promotion host constraints for that slice:
  - `Runtime permissions profile = read_only_lane = canonical Directive Workspace state plus linked Runtime artifacts through the existing DW thin-host reader; write_lane = none`
  - `Safe output scope = mini-swe route seam-review page plus thin-host detail payload and package/checker snapshots only; no promotion-record creation, no registry acceptance, no host integration writes, no runtime execution`
- Still out of scope after making this slice explicit:
  - host-facing promotion record creation
  - registry acceptance
  - host integration
...
- Explicit bounded pre-promotion profile/checker family for that slice:
  ...
  - `Proof shape = dw_web_host_seam_review_snapshot/v1`
  - `Primary host checker = npm run check:directive-dw-web-host-runtime-seam-review`
  - `Focused profile/checker decision = npm run check:directive-real-mini-swe-agent-runtime-route-v0-dw-web-host-profile-checker-decision`
  - `Contract path = shared/contracts/dw-web-host-seam-review-guard.md`
- Still out of scope after making this slice explicit:
  - host-facing promotion record creation
  - registry acceptance
...
- Explicit bounded pre-promotion profile/checker family for that slice:
  ...
  - `Proof shape = dw_web_host_seam_review_snapshot/v1`
  - `Primary host checker = npm run check:directive-dw-web-host-runtime-seam-review`
  - `Focused implementation checker = npm run check:directive-real-mini-swe-agent-runtime-route-v0-dw-web-host-runtime-implementation-slice`
  - `Contract path = shared/contracts/dw-web-host-seam-review-guard.md`
- Explicit bounded host constraints for that slice:
  - `Runtime permissions profile = read_only_lane = canonical Directive Workspace state plus linked Runtime artifacts through the existing DW thin-host reader; write_lane = none`
  - `Safe output scope = mini-swe route seam-review page plus thin-host detail payloads and implementation-bundle links only; no promotion-record creation, no registry acceptance, no host integration writes, no runtime execution`
- Still out of scope after making this slice explicit:
  - host-facing promotion record creation
  - registry acceptance
  - host integration
...
- Reviewed target host: `Directive Workspace web host (frontend/ + hosts/web-host/)`
- Reviewed decision: `manual_real_mini_swe_agent_runtime_route_dw_web_host_promotion_record_opened`
- Decision reason: the bounded DW web-host implementation slice is now real and checker-backed for this exact case, the compile-contract and promotion-input bundle are explicit, and the one-case mini-swe route DW web-host manual promotion guard is now explicit while registry acceptance, host integration, runtime execution, and promotion automation all remain closed.
- Evidence already satisfied:
  ...
  - the mini-swe route DW web-host manual promotion guard and focused promotion checker are explicit
- Remaining blockers:
...
- Explicit bounded manual promotion profile/checker family for that slice:
  ...
  - `Contract path = shared/contracts/real-mini-swe-agent-runtime-route-dw-web-host-runtime-promotion-guard.md`
- Explicit bounded promotion host constraints for that slice:
  - `Runtime permissions profile = read_only_lane = canonical Directive Workspace state plus linked Runtime artifacts through the existing DW thin-host reader; write_lane = none`
  - `Safe output scope = mini-swe route seam-review page plus thin-host detail payloads only; no execution, no host integration writes, no callable activation`
...
- Host-facing promotion record: `runtime/07-promotion-records/2026-04-02-dw-real-mini-swe-agent-runtime-route-v0-2026-03-25-promotion-record.md`
