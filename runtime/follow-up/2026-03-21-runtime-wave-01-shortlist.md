...
Open the next bounded Runtime execution wave after the current Architecture cycle closed.

Wave 01 remains the first real Runtime runtime cycle, but it is now explicitly system-first.

Before opening broader runtime candidates, Runtime should close the operational debt that would make later runtime slices noisy, ambiguous, or unsafe.
...
- the adoption target is clearly Runtime, not Architecture
...
- Current state:
  - bounded Runtime runtime slice completed
...
- Why it stays first:
  ...
  - it proves Runtime can promote one bounded callable surface without treating the whole upstream repo as runtime truth
- Rule:
  - do not use promptfoo completion as justification to skip remaining Runtime system cleanup

## Active System Queue

### 1) Runtime System Bundle 02: mirror and package boundary inventory
- Status:
  - completed on 2026-03-21
- Why this is active now:
  - Runtime core and shared-lib mirrors still exist in Mission Control
  - package consumption remains explicitly deferred in current docs
  - before opening more runtime surfaces, Runtime needs a clean inventory of what is canonical, mirrored, and still host-only
- Expected outputs:
  - one Runtime-owned inventory/decision artifact
  - one explicit keep/move/defer table for package/import cutover
  - no speculative runtime cutover yet

### 2) Runtime System Bundle 03: source-pack catalog and activation cleanup
- Status:
  - completed on 2026-03-21
- Why this is next:
  - source-pack readiness is now enforced for runtime resolution
  - Runtime still needs a cleaner distinction between:
    - live runtime packs
    - cutover-only packs
...
### 3) Runtime System Bundle 04: promotion-profile family normalization
- Status:
  - completed on 2026-03-21
- Why this remains in queue:
  - promptfoo exposed that Runtime has more than one promotion-proof family
...
- Current state:
  - bounded Runtime runtime slice completed on 2026-03-21
  - promotion record and registry entry exist
  - dedicated `browser_smoke_guard/v1` contract family is in place
- Why queued:
  - clear Runtime runtime value
  - host-fit is strong
...
- Why queued:
  - important Runtime lifecycle/governance value
...
- Why queued:
  ...
  - Runtime-owned pack is currently missing the runnable CLI artifact required by the host (`packages/cli/dist/index.js`)
- Rule:
...
2. completed: Runtime System Bundle 02
3. completed: Runtime System Bundle 03
4. completed: Runtime System Bundle 04
...
Wave 01 planning and system cleanup use the lighter Runtime planning bundle first:
