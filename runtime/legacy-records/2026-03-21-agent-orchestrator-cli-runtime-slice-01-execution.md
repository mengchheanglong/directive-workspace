...
Prove one narrow Mission Control host use case for the Runtime-owned `agent-orchestrator` pack after the CLI precondition proof:
- `ao status --json` against a generated local runtime config

## Boundaries

Allowed:
- temp-copy build of the Runtime-owned pack
