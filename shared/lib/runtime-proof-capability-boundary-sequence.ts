...
type DirectiveRuntimeProofCapabilityBoundarySequenceBaseResult = {
  sequenceId: string;
  caseId: string;
  resumed: boolean;
...
export type DirectiveRuntimeProofCapabilityBoundarySequenceSuccessResult =
  DirectiveRuntimeProofCapabilityBoundarySequenceBaseResult & {
    ok: true;
...
export function runDirectiveRuntimeProofCapabilityBoundaryTwoStepSequence(
  ...
  try {
    ...
    if (record.steps[1].targetPath !== step1ActionResult.runtimeProofRelativePath) {
      throw new Error(
        `invalid_input: declared step 2 target ${record.steps[1].targetPath} does not match step 1 opened Runtime proof ${step1ActionResult.runtimeProofRelativePath}`,
      );
    }
