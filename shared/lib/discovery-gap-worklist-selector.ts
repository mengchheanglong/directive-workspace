...
function resolveDirectiveRelativePath(directiveRoot: string, inputPath: string) {
  ...
  if (normalizedInput.length === 0) {
    throw new Error("invalid_input: worklistPath is required");
  }
  ...
  if (absolutePath !== root && !absolutePath.startsWith(normalizedRootPrefix)) {
    throw new Error("invalid_input: worklistPath must stay within directive-workspace");
  }
...
function readDiscoveryGapWorklist(input: {
  ...
  const worklistAbsolutePath = normalizePath(
    path.join(input.directiveRoot, input.worklistRelativePath),
  );
  ...
  const parsed = JSON.parse(
    fs.readFileSync(worklistAbsolutePath, "utf8"),
  ) as DiscoveryGapWorklist;
