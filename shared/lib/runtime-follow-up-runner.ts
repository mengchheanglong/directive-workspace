...
export function runDirectiveRuntimeFollowUpWithRunner(input: {
  ...
  if (existing?.checkpointStage === "after_action" && existing.actionResult) {
    ...
    if (resumed) {
      appendRunnerEvent({
        ...
        message: "Runner resumed from after_action checkpoint without re-executing the opener.",
      });
    }
    appendRunnerEvent({
      directiveRoot,
  ...
  appendRunnerEvent({
    ...
    occurredAt: beforeActionAt,
    message: "Runner checkpointed before calling the Runtime follow-up opener.",
  });
  ...
  try {
    ...
    appendRunnerEvent({
      ...
      message: "Runner checkpointed after the Runtime follow-up opener completed.",
    });
