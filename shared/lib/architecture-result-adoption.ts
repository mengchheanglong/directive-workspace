...
function resolveAdoptedRelativePath(input: {
  ...
  return path.posix.join("architecture/03-adopted", adoptedFileName);
}
...
export function readDirectiveArchitectureAdoptionDetail(input: {
  ...
  if (!adoptedRelativePath.startsWith("architecture/03-adopted/")) {
    throw new Error("invalid_input: adoptionPath must point to architecture/03-adopted/");
  }
