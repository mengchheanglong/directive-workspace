...
Purpose:
- enforce bounded skill-lifecycle import evidence on Forge promotion records before callable skill-lifecycle claims
...
Scope:
- applies to Forge promotion records whose `Quality gate profile` is `skill_lifecycle_guard/v1`
...
- promotion record declares `Primary host checker: npm run check:directive-skills-manager-forge`
...
Decision rules:
1. A bounded skill-lifecycle lane may claim `pass` only when the import smoke artifact proves the `Skills Lifecycle Operator` can be imported from the Forge-owned pack and re-synced safely.
