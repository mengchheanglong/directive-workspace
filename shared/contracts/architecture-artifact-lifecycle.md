...
- Forge handoff note (explicit: yes with ref, or no with threshold justification)
...
## Forge handoff is a transition, not a state

`Forge-handoff` is not a fifth Architecture artifact state.
...
- the Forge threshold question is answered "no" - the mechanism would not be valuable without a runtime surface

Track Forge handoff through:
- the adopted Architecture record
- `shared/contracts/architecture-to-forge.md`
- any downstream Forge record that accepts the handoff
...
### adopted -> Forge handoff

Requirements:
- Architecture has captured its own value as a product-owned artifact
- the remaining work is runtime operationalization
- the Forge threshold check confirms the mechanism needs a runtime surface
- an explicit `architecture-to-forge` handoff record exists or is explicitly queued as the next step
...
5. Does it describe runtime work remaining after Architecture is done? -> still `adopted`, with a Forge handoff note
...
- Status class: per `architecture-completion-rubric`
- Forge threshold check: yes/no with reasoning
...
2. Is the usefulness level correct? Check whether the mechanism improves the engine's own source-consumption ability (meta), improves the framework structure (structural), or provides runtime value (direct/Forge).
...
- Forge handoff is a transition note on an adopted record, not a separate state.
...
- Works with: `architecture-to-forge` (Forge handoff transition)
