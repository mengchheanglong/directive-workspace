...
Define the bounded Runtime proof for `agent-orchestrator` without widening the host into dashboard, mobile, or full plugin-platform runtime truth.

## Pass Conditions

- the latest AO CLI smoke report is `ok: true`
- the smoke uses a temp copy of the Runtime-owned source pack rather than mutating the canonical source-pack root
...
- `npm run runtime:agent-orchestrator:smoke`
...
- leaving temp build outputs in the canonical Runtime source-pack root as the proof artifact
