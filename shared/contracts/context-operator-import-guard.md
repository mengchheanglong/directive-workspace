...
Purpose:
- enforce bounded context-operator import evidence on Forge promotion records before callable context-planning/review claims
...
Scope:
- applies to Forge promotion records whose `Quality gate profile` is `context_operator_import_guard/v1`
...
- promotion record declares `Primary host checker: npm run check:directive-arscontexta-forge`
