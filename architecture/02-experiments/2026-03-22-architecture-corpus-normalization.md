...
Status: executed
Revision: 2 — corrected from initial baseline (see correction log at end)
...
- All 24 records in `architecture/03-adopted/` as of 2026-03-22
- All 31 records in `architecture/05-reference-patterns/`
- All 8 records in `architecture/04-deferred-or-rejected/` (+ README)
- 52 experiment records in `architecture/02-experiments/` (sampled, not exhaustively classified)
...
### Wave 3 / recent adoptions (2026-03-21 – 2026-03-22)
...
| `architecture-maturity-bundle` | internal | **meta** | product_materialized | n/a | n/a | n/a | **yes — first record with evidence** | current doctrine |
...
|---|---|---|
| `agent-orchestrator-pattern-split` | pattern | no — context only, value already absorbed by Architecture lane-split thinking |
| `hermes-agent-surviving-patterns` | analysis-notes | no — value absorbed by compaction-fidelity contract |
| `codegraphcontext-analysis-patterns` | analysis-notes | no — value absorbed by index-query-state-boundary contract |
| `impeccable-frontend-guardrails` | policy | no — narrower subset, value absorbed by review-guardrails contract |
| `arscontexta-context-patterns` | analysis-notes | no — context only |
| `cross-source-theory-paper-patterns` | analysis-notes | no — context only |
| `superpowers-workflow-discipline` | pattern | no — context only |
| `citation-set-fallback-policy` | policy | no — value absorbed by citation-set-fallback contract |
| `stage-evidence-citation-handoff-contract` | contract-draft | **retired** — fully absorbed by shared schemas |
| `structured-output-fallback-parser-policy` | policy | no — value absorbed by shared contract |
| `integration-proof-template-generator-policy` | policy | no — value absorbed by shared lib |
| `impeccable-reanalysis-policy-bundle-02` | analysis-notes | no — context only |
| `impeccable-review-checklist-policy` | policy | no — value absorbed by review-guardrails contract |
| `paper2code-stage-schema-normalization` | policy | no — value absorbed by shared schemas |
| `celtrix-stack-signal-policy` | policy | no — value absorbed by intake-stack-signals contract |
| `hermes-compaction-fidelity-policy` | policy | no — value absorbed by compaction-fidelity contract |
| `codegraphcontext-state-boundary-policy` | policy | no — value absorbed by index-query-state-boundary contract |

### Governance/infrastructure reference patterns (previously omitted)

| Reference pattern | Type | Should it promote? |
|---|---|---|
| `directive-v1-architecture-recheck-lock` | governance lock | no — locked baseline record, value is the lock itself |
| `future-candidates-agency-agents-arscontexta` | parked-candidates note | no — partially superseded by arscontexta-context-patterns; retains future-candidate context |
| `dw-cross-source-wave-01-integration-contract-artifact` | generated artifact | no — machine-generated integration contract for cross-source wave; context only |
| `dw-cross-source-wave-01-proof-checklist-artifact` | generated artifact | no — machine-generated proof checklist for cross-source wave; context only |
| `promptfoo-evaluation-patterns` | pattern extraction | no — evaluation discipline patterns; value partially absorbed by evaluator contract |
| `agent-lab-orchestration-allowlist-policy` | policy | no — value absorbed by source-pack-curation-allowlist contract |
...
- None require urgent promotion — the shared contract layer already captures the surviving value
- **Total reference patterns: 31** (8 promoted + 22 reference-only + 1 retired)

## Classification of deferred/rejected records

The `architecture/04-deferred-or-rejected/` directory contains 8 records (+ README). These were not classified in the initial normalization pass. The recheck record (`recheck-deferred-rejected-v2`) already contains good per-candidate classification.
...
### Finding 3: Usefulness level was never classified — now it is
...
Compare:
- `gpt-researcher` experiment: 200+ lines of detailed extraction evidence with specific file references, contract shapes, and validation rules — strongest record
- `discovery-gap-driven-priority-loop`: 30 lines, claim + bounded change + proof method — adequate but thin
- `agentics-slice-2-execution`: mostly commands-run + pass/fail — execution evidence only, no analysis depth
...
6. **The Architecture corpus is now self-aware** — it knows what it has, at what level, with what gaps

### What this record does NOT do

- Rewrite pre-doctrine records (not needed — the lifecycle contract exempts them)
...
The Architecture corpus is structurally sound but pre-doctrine. The main gap is not quality — it's classification and self-awareness. This normalization record closes that gap for the current corpus. The `architecture-artifact-lifecycle` contract prevents the gap from recurring in future work.
