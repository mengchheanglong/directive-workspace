...
function readArchitectureHandoffArtifact(input: {
  ...
  if (!input.handoffRelativePath.startsWith("architecture/01-experiments/")) {
    throw new Error("invalid_input: handoffPath must point to architecture/01-experiments/");
  }
  ...
  return fs
    ...
    .map((entry) =>
      readArchitectureHandoffArtifact({
        directiveRoot,
        handoffRelativePath: `architecture/01-experiments/${entry.name}`,
      }))
