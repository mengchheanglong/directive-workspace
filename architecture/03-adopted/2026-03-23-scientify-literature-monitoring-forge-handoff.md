...
- Forge should still keep the workflow design host-neutral enough that later hosts can adopt it

## Proposed runtime surface

- Proposed runtime surface:
  - Forge-owned bounded workflow or source-pack for recurring literature monitoring
...
- Keep the host-facing workflow reversible and bounded; no broad autonomous research pipeline in the first Forge slice.

## Required proof

- one bounded Forge proof that the monitoring workflow can:
...
- one Architecture-to-Forge parity proof that the runtime surface still respects:
...
- Forge-side bounded proof/evaluator gate for digest quality and degraded-state handling
...
- If the Forge slice fails, keep the Architecture partition and promotion-quality patterns
...
- Improvement summary:
  - the handoff is now based on an explicit mixed-value partition rather than ad hoc “maybe Forge later” language
  - Forge receives a narrower reusable workflow candidate, not the whole plugin
- Value handed to Forge:
...
- Meta-useful: `yes`
- Reason:
  - the runtime candidate is direct-useful, but the handoff itself also improves Directive Workspace's handoff quality by showing how mixed-value sources can yield a precise Forge follow-up without losing Architecture-owned value
