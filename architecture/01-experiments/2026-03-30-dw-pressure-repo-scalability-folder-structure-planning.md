...
- `architecture/01-experiments/` contains `36,603` files, or `98.9%` of `architecture/`.
- Hidden virtualenv trees inside `architecture/01-experiments/` contain `36,220` files, or `99.0%` of `02-experiments/`.
- `runtime/` contains `11,212` files, of which `runtime/source-packs/` contains `10,915` (`97.4%`).
- `sources/` contains `12,950` files split across `01-intake/` and `05-deferred-or-rejected/`.
...
- `runtime/` lane record surfaces such as `00-follow-up/`, `02-records/`, `03-proof/`, `04-capability-boundaries/`, `05-promotion-readiness/`, `legacy-records/`, `07-promotion-records/`, `08-registry/`, and runtime run records under `standalone-host/`
...
### 1. `architecture/01-experiments/` is no longer a readable active experiment surface
...
- `architecture/01-experiments/.venv-gptr-smoke`
- `architecture/01-experiments/.venv-mini-swe-slice3`

now account for almost the entire file volume of the active experiment surface.

That causes three problems:
- it makes `architecture/01-experiments/` look like a giant corpus when most of its volume is actually local execution scratch
...
The split between `01-intake/` and `05-deferred-or-rejected/` is already understandable, and changing it now would risk sliding into archive cleanup rather than a bounded structural correction.
...
### Candidate A: move lane-local virtualenv scratch out of `architecture/01-experiments/`
...
Open one bounded structural slice next to create a dedicated non-authoritative scratch/workdir surface and relocate the two hidden `.venv-*` directories out of `architecture/01-experiments/`.
...
It is one bounded scratch-surface relocation slice for the two hidden virtualenv directories currently distorting `architecture/01-experiments/`.
