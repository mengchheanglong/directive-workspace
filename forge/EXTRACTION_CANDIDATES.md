# Forge Core Extraction Candidates

Current host:
- `C:\Users\User\.openclaw\workspace\mission-control`

Goal:
- identify Forge logic that can move into Directive Workspace without dragging host-specific runtime code with it
...
- extracted into Forge successfully
...
Reason:
- pure parser
- useful to Forge product layer and host layer
...
Reason:
- these are Forge policy rules, not host storage rules

### 4. Decision contract normalization

Current canonical location:
- `C:\Users\User\.openclaw\workspace\directive-workspace\forge\core\decision-contract.ts`
...
Reason:
- this is Forge contract logic, not Mission Control storage logic
...
Reason:
- these are Forge workflow contracts, not host persistence logic

### 6. Presentation and lifecycle summary contract

Current canonical location:
- `C:\Users\User\.openclaw\workspace\directive-workspace\forge\core\presentation-contract.ts`
...
Reason:
- these are Forge presentation and derived-state rules
...
Reason:
- these are Forge proof contract rules
- host service should only handle filesystem/report side effects

### 8. Capability transition patch contract

Current canonical location:
- `C:\Users\User\.openclaw\workspace\directive-workspace\forge\core\capability-patch-contract.ts`
...
Reason:
- these are Forge transition rules, not host persistence primitives
