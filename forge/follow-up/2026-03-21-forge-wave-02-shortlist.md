...
Open the next Forge wave only after correcting the queue around the real remaining system debt.

Wave 01 proved three bounded runtime lanes and corrected the false-live `agent-orchestrator` state.  
Wave 02 should not open another pack blindly. It should first align host import behavior and legacy live-runtime accounting with the Forge catalog and promotion model.

## Selection Rule

A Wave 02 item qualifies only if:
- it reduces a real Forge product/host inconsistency
...
### 1) Forge System Bundle 05: import-source policy alignment
- Status:
  - completed
- Why this is first:
  - the Forge catalog now distinguishes `live_runtime`, `follow_up_only`, and `reference_only`
...
- Completed result:
  - Forge now owns `IMPORT_SOURCE_POLICY.json`
...
### 2) Forge System Bundle 06: legacy live-runtime normalization
- Status:
  - completed
- Why this is next:
  - `agency-agents` and `desloppify` are still live in the host
  - they predate the newer Forge proof/promotion/registry discipline
  - Forge needs an explicit legacy-normalization decision instead of silent grandfathering
- Expected outputs:
  - one keep/normalize table for each legacy live pack
  - either a real Forge proof/promotion trail or an explicit legacy live-runtime contract
  - host validation that every `live_runtime` pack has declared lifecycle accounting
- Completed result:
  - `agency-agents` and `desloppify` now have explicit Forge record/proof/promotion/registry artifacts
  - Forge now owns `LIVE_RUNTIME_ACCOUNTING.json`
...
- Why next:
  ...
  - already cutover-complete under Forge ownership
...
- Status:
  - blocked follow-up
- Re-entry rule:
  - remains blocked until the Forge-owned pack contains `packages/cli/dist/index.js`
...
- Reason:
  - surviving value is utility/reference level, not a higher-priority Forge lane
...
1. completed: Forge System Bundle 05
2. completed: Forge System Bundle 06
