...
Any future Forge slice based on this source must prove:
- bounded operational value for the actual Directive host workflow
- rollback path
- no dependency on `agent-lab` path ownership
- the Forge-owned source pack can reproducibly produce and run the AO CLI entrypoint from `packages/cli/dist/index.js`
...
- the bounded AO CLI status smoke is now proven in:
  - `C:\Users\User\.openclaw\workspace\directive-workspace\forge\records\2026-03-21-agent-orchestrator-cli-runtime-slice-01-proof.md`
- the host-adapter decision is now closed in:
  - `C:\Users\User\.openclaw\workspace\directive-workspace\forge\records\2026-03-21-agent-orchestrator-host-adapter-decision.md`
...
- any retained runtime behavior is re-homed into product-owned Forge assets or explicitly dropped
