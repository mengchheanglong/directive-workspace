"""Tests for desloppify.languages.python.detectors.complexity — Python complexity signals."""
...
class TestComputeMaxParams:
    ...
    def test_nested_parens_handled(self):
        ...
        content = (
            "def typed(a: dict[str, int], b: tuple[int, ...], c, d, e, f, g, h): pass\n"
        )
        lines = content.splitlines()
        result = compute_max_params(content, lines)
        assert result is not None
...
class TestDetectIndentUnit:
    def test_4_space_indent(self):
        ...
        assert complexity_mod._detect_indent_unit(lines) == 4
    ...
    def test_empty_file(self):
        lines = []
        assert complexity_mod._detect_indent_unit(lines) == 4  # default
