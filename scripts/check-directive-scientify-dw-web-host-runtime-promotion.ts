...
const CHECKER_ID = "directive_scientify_dw_web_host_runtime_promotion";
...
const EXPECTED_PROMOTION_DECISION =
  "manual_scientify_dw_web_host_promotion_record_opened";
...
async function main() {
  ...
  assert.ok(fs.existsSync(promotionRecordAbsolutePath), "Scientify DW web-host promotion record should exist");
  ...
  assert.ok(readinessFocus?.ok, "Scientify DW web-host promotion-readiness focus should resolve");
  assert.ok(promotionRecordFocus?.ok, "Scientify DW web-host promotion-record focus should resolve");
  ...
  assert.equal(
    ...
    "runtime/02-records/2026-03-24-dw-live-scientify-engine-pressure-2026-03-24-runtime-record.md",
  );
  ...
  assert.equal(
    ...
    "scientify_dw_web_host_manual_promotion_guard/v1",
  );
  assert.equal(
    readBullet(promotionRecordContent, "Promotion profile family"),
    "bounded_scientify_dw_web_host_manual_promotion",
  );
  assert.equal(
    readBullet(promotionRecordContent, "Proof shape"),
    "scientify_dw_web_host_manual_promotion_snapshot/v1",
  );
  assert.equal(
  ...
  assert.ok(
    ...
    "Scientify DW web-host registry acceptance should remain unopened after the manual promotion-record slice",
  );
  ...
  await withDirectiveFrontendCheckServer({ directiveRoot: DIRECTIVE_ROOT }, async (handle) => {
    ...
    const queueCase = Array.isArray(snapshot.queue?.entries)
      ? snapshot.queue.entries.find((entry: any) =>
        entry?.candidate_id === "dw-live-scientify-engine-pressure-2026-03-24")
      : null;
