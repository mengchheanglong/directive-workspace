...
function main() {
  ...
  assert.equal(report.runtimeQueue.summary.alreadyPromotedManualCycleCount, 14);
  assert.equal(report.runtimeQueue.summary.readyForManualPromotionSeamDecisionCount, 0);
  ...
  assert.equal(report.coordinationContext.recommendTaskCount, 0);
  ...
  assert.ok(
    ...
    "Expected the loop selector to explain that no Runtime recommendation frontier remains",
  );
