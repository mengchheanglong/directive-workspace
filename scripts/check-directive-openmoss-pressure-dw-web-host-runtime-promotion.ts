...
const CHECKER_ID = "directive_openmoss_pressure_dw_web_host_runtime_promotion";
...
const EXPECTED_PROMOTION_DECISION =
  "manual_openmoss_pressure_dw_web_host_promotion_record_opened";
...
async function main() {
  ...
  assert.ok(fs.existsSync(promotionRecordAbsolutePath), "OpenMOSS pressure promotion record should exist");
  ...
  assert.ok(readinessFocus?.ok, "OpenMOSS pressure promotion-readiness focus should resolve");
  assert.ok(promotionRecordFocus?.ok, "OpenMOSS pressure promotion-record focus should resolve");
  ...
  assert.equal(
    ...
    "openmoss_pressure_dw_web_host_manual_promotion_guard/v1",
  );
  assert.equal(
    readBullet(promotionRecordContent, "Promotion profile family"),
    "bounded_openmoss_pressure_dw_web_host_manual_promotion",
  );
  assert.equal(
    readBullet(promotionRecordContent, "Proof shape"),
    "openmoss_pressure_dw_web_host_manual_promotion_snapshot/v1",
  );
  assert.equal(
  ...
  assert.ok(
    ...
    "OpenMOSS pressure registry acceptance should remain unopened after the manual promotion-record slice",
  );
  ...
  await withDirectiveFrontendCheckServer({ directiveRoot: DIRECTIVE_ROOT }, async (handle) => {
    ...
    const queueCase = Array.isArray(snapshot.queue?.entries)
      ? snapshot.queue.entries.find((entry: any) =>
        entry?.candidate_id === "dw-pressure-openmoss-architecture-loop-2026-03-26")
      : null;
