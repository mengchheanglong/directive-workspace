...
const EXPECTED_PROMOTION_DECISION =
  "manual_openmoss_dw_web_host_promotion_record_opened";
...
async function main() {
  ...
  assert.ok(fs.existsSync(promotionRecordAbsolutePath), "OpenMOSS promotion record should exist");
  ...
  assert.ok(readinessFocus?.ok, "OpenMOSS promotion-readiness focus should resolve");
  assert.ok(promotionRecordFocus?.ok, "OpenMOSS promotion-record focus should resolve");
  ...
  assert.equal(
    ...
    "runtime/02-records/2026-03-26-dw-mission-openmoss-runtime-orchestration-2026-03-26-runtime-record.md",
  );
  assert.equal(
  ...
  assert.equal(
    ...
    "openmoss_dw_web_host_manual_promotion_guard/v1",
  );
  assert.equal(
    readBullet(promotionRecordContent, "Promotion profile family"),
    "bounded_openmoss_dw_web_host_manual_promotion",
  );
  assert.equal(
    readBullet(promotionRecordContent, "Proof shape"),
    "openmoss_dw_web_host_manual_promotion_snapshot/v1",
  );
  assert.equal(
  ...
  assert.ok(
    ...
    "OpenMOSS registry acceptance should remain unopened after the manual promotion-record slice",
  );
