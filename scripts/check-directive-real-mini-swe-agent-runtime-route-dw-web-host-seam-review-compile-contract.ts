...
const CHECKER_ID = "directive_real_mini_swe_agent_runtime_route_dw_web_host_seam_review_compile_contract";
...
function main() {
  ...
  assert.ok(focus?.ok, "mini_swe_route_dw_web_host_compile_contract_focus_should_resolve");
  ...
  assertIncludes(compileContractContent, PROMOTION_READINESS_PATH, "compile_contract_readiness_path");
  assertIncludes(compileContractContent, PROMOTION_SPEC_PATH, "compile_contract_promotion_spec_path");
  assertIncludes(compileContractContent, INPUT_PACKAGE_PATH, "compile_contract_input_package_path");
  assertIncludes(compileContractContent, DW_WEB_HOST_GUARD_PATH, "compile_contract_guard_path");
  assertIncludes(compileContractContent, "`frontend/src/app.ts`", "compile_contract_frontend_file");
  assertIncludes(compileContractContent, "`hosts/web-host/server.ts`", "compile_contract_server_file");
  assertIncludes(compileContractContent, "`hosts/web-host/data.ts`", "compile_contract_data_file");
  assertIncludes(
    compileContractContent,
  ...
  assertIncludes(compileContractContent, "promotion-record creation", "compile_contract_non_authorization_promotion_record");
  assertIncludes(compileContractContent, "registry acceptance", "compile_contract_non_authorization_registry");
  assertIncludes(compileContractContent, "runtime execution", "compile_contract_non_authorization_execution");
  assertIncludes(compileContractContent, "host integration writes", "compile_contract_non_authorization_integration");
  assertIncludes(
    compileContractContent,
  ...
  assertIncludes(
    compileContractContent,
    "npm run check:directive-real-mini-swe-agent-runtime-route-dw-web-host-seam-review-compile-contract",
