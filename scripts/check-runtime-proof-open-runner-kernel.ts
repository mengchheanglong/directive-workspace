...
import { runDirectiveRuntimeProofOpenWithRunner } from "../runtime/lib/runtime-proof-open-runner.ts";
import {
  copyRelativeFile,
  extractOpenedBy,
  readJson,
...
function seedRuntimeProofDirectiveRoot(directiveRoot: string) {
  ...
  assert.ok(liveFocus?.ok, `Live Runtime proof state did not resolve for ${CASE_UNDER_TEST.candidateId}`);
  ...
  return {
    ...
    openedBy: extractOpenedBy(
      fs.readFileSync(path.join(DIRECTIVE_ROOT, CASE_UNDER_TEST.runtimeProofPath), "utf8"),
    ),
  };
}

function copyDownstreamProofChain(directiveRoot: string, liveFocus: NonNullable<ReturnType<typeof resolveDirectiveWorkspaceState>["focus"]>) {
  for (const relativePath of uniqueRelativePaths([
...
function assertResolvedStateMatchesLive(input: {
  ...
  assert.ok(tempFocus?.ok, "Generated Runtime proof state did not resolve");
...
function scenarioDirectBaseline() {
  withTempDirectiveRoot({ prefix: "directive-runtime-proof-runner-" }, (directiveRoot) => {
    ...
    const result = openDirectiveRuntimeRecordProof({
      ...
      approved: true,
      approvedBy: seeded.openedBy,
    });

    assert.equal(result.created, true, "Direct proof opener should create the proof artifact");
    assert.equal(
      fs.readFileSync(path.join(directiveRoot, CASE_UNDER_TEST.runtimeProofPath), "utf8"),
      seeded.liveProofMarkdown,
    );

    copyDownstreamProofChain(directiveRoot, seeded.liveFocus);
...
function scenarioFreshRunner() {
  withTempDirectiveRoot({ prefix: "directive-runtime-proof-runner-" }, (directiveRoot) => {
    ...
    assert.ok(runnerRecord, "Fresh proof runner record should exist");
...
function scenarioInterruptedBeforeActionThenResumed() {
  withTempDirectiveRoot({ prefix: "directive-runtime-proof-runner-" }, (directiveRoot) => {
    ...
    assert.equal(
      ...
      false,
      "Interrupted-before-action run must not create the proof artifact",
    );

    const resumed = runDirectiveRuntimeProofOpenWithRunner({
      directiveRoot,
      runnerId,
      runtimeRecordPath: CASE_UNDER_TEST.runtimeRecordPath,
      approved: true,
    ...
    assert.ok(runnerRecord, "Resumed proof runner record should exist");
    assert.equal(runnerRecord.lifecycleState, "completed");
...
function scenarioInterruptedAfterActionThenResumed() {
  withTempDirectiveRoot({ prefix: "directive-runtime-proof-runner-" }, (directiveRoot) => {
    ...
    assert.ok(runnerRecord, "After-action proof runner record should exist");
    assert.equal(runnerRecord.lifecycleState, "completed");
    assert.equal(runnerRecord.attempts, 1);
    ...
    }).events.filter((event) => event.eventType === "runtime_proof_opened");
    assert.equal(mirrorEvents.length, 1, "Resume from after_action must not re-open proof");
...
function scenarioApprovalAndStaleHeadGuards() {
  withTempDirectiveRoot({ prefix: "directive-runtime-proof-runner-" }, (directiveRoot) => {
    ...
    assert.throws(
      ...
      /explicit approval/i,
      "Proof runner should keep the explicit approval guard",
    );
    assert.equal(
      readDirectiveActionRunnerRecord({
        directiveRoot,
    ...
    runDirectiveRuntimeProofOpenWithRunner({
      directiveRoot,
      runnerId: "runtime-proof-runner-success-first",
      runtimeRecordPath: CASE_UNDER_TEST.runtimeRecordPath,
      approved: true,
      approvedBy: seeded.openedBy,
    });

    const staleRunnerId = "runtime-proof-runner-stale-head";
    assert.throws(
      () =>
        runDirectiveRuntimeProofOpenWithRunner({
          directiveRoot,
      ...
      "Proof runner should preserve stale-head guard behavior",
    );
    ...
    assert.ok(failedRunnerRecord, "Failed stale-head proof runner should persist state");
