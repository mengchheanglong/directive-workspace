...
import {
  renderStandaloneResearchVaultHostConsumptionReport,
  resolveStandaloneResearchVaultHostConsumptionReportPath,
} from "../hosts/standalone-host/runtime-lane.ts";
import { resolveDirectiveWorkspaceState } from "../engine/state/index.ts";

const DIRECTIVE_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const GENERATED_AT = "2026-04-07T09:15:00.000Z";
...
async function main() {
  ...
  try {
    ...
    const reportPath = resolveStandaloneResearchVaultHostConsumptionReportPath({
      candidateId: descriptor.candidateId,
      generatedAt: GENERATED_AT,
    });
    const absoluteReportPath = path.join(DIRECTIVE_ROOT, reportPath);
    const report = renderStandaloneResearchVaultHostConsumptionReport({
      generatedAt: GENERATED_AT,
      descriptor,
      primaryChecker: "npm run check:standalone-research-vault-host-adapter",
    });
