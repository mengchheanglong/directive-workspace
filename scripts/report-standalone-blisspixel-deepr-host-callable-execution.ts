...
import {
  renderStandaloneBlisspixelDeeprHostCallableExecutionReport,
  resolveStandaloneBlisspixelDeeprHostCallableExecutionReportPath,
} from "../hosts/standalone-host/runtime-lane.ts";

const DIRECTIVE_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const GENERATED_AT = "2026-04-07T10:15:00.000Z";
...
async function main() {
  ...
  try {
    const invocationResult = await host.invokeBlisspixelDeeprDescriptorCallable({
      action: "summarize_descriptor",
      includeOpenDecisions: true,
      executedAt: GENERATED_AT,
    });
    const reportPath = resolveStandaloneBlisspixelDeeprHostCallableExecutionReportPath({
      candidateId: invocationResult.candidateId,
      generatedAt: GENERATED_AT,
    });
    const absoluteReportPath = path.join(DIRECTIVE_ROOT, reportPath);
    const report = renderStandaloneBlisspixelDeeprHostCallableExecutionReport({
      generatedAt: GENERATED_AT,
      invocationResult,
