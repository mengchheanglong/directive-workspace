...
const CHECKER_ID = "directive_real_mini_swe_agent_runtime_route_v0_dw_web_host_promotion_input_package";
const CANDIDATE_ID = "dw-real-mini-swe-agent-runtime-route-v0-2026-03-25";
...
function main() {
  ...
  assert.ok(focus?.ok, "mini_swe_route_dw_web_host_input_package_focus_should_resolve");
  ...
  assertIncludes(readinessContent, "## Directive Workspace web-host promotion-input package", "readiness_input_package_section");
  assertIncludes(readinessContent, INPUT_PACKAGE_PATH, "readiness_input_package_path");
  assertIncludes(
    readinessContent,
  ...
  assertIncludes(compileContractContent, INPUT_PACKAGE_PATH, "compile_contract_input_package_path");
  assertIncludes(compileContractContent, "Focused input-package checker:", "compile_contract_input_checker_label");
  assertIncludes(
    compileContractContent,
    "npm run check:directive-real-mini-swe-agent-runtime-route-v0-dw-web-host-promotion-input-package",
    "compile_contract_input_checker",
  );

  assertIncludes(inputPackageContent, "Status: `explicit_non_promoting_dw_web_host_input_bundle`", "input_package_status");
  assertIncludes(inputPackageContent, COMPILE_CONTRACT_PATH, "input_package_compile_contract_path");
  assertIncludes(inputPackageContent, DW_WEB_HOST_GUARD_PATH, "input_package_guard_path");
  assertIncludes(
    inputPackageContent,
  ...
  assertIncludes(inputPackageContent, "Quality gate profile = dw_web_host_seam_review_guard/v1", "input_package_quality_gate_profile");
  assertIncludes(inputPackageContent, "Primary host checker = npm run check:directive-dw-web-host-runtime-seam-review", "input_package_primary_host_checker");
  assertIncludes(
    inputPackageContent,
    "Focused input-package checker = npm run check:directive-real-mini-swe-agent-runtime-route-v0-dw-web-host-promotion-input-package",
    "input_package_focused_checker",
  );
  assertIncludes(inputPackageContent, "host-facing promotion record creation", "input_package_non_authorization_promotion_record");
  assertIncludes(inputPackageContent, "registry acceptance", "input_package_non_authorization_registry");
  assertIncludes(inputPackageContent, "host integration rollout", "input_package_non_authorization_integration");
  assertIncludes(inputPackageContent, "runtime execution", "input_package_non_authorization_execution");

  assertIncludes(guardContent, "This guard applies only to read-only seam review.", "guard_read_only_scope");
