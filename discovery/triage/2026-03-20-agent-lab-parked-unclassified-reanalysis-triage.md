...
|---|---|---|---|---|---|---|---|---|---|
| `al-parked-autoresearch` | Useful autonomous-loop discipline already partly extracted; needs delta-only review under current Doctrine | route for focused Architecture+Forge re-analysis | Architecture + Forge follow-up | experiment | high | high | medium | medium | yes |
...
| `al-parked-cli-anything` | Command mediation remains interesting but safety/abuse surface is non-trivial | route to Forge with strict guardrail-first scope | Forge | defer-to-experiment | medium | medium | high | medium | yes (bounded) |
...
| `al-parked-desloppify` | Small, practical quality utility with clear operational value | route to Forge utility hardening | Forge | accept-for-forge-follow-up | high | high | low | low | yes |
| `al-parked-hermes-agent` | Strong contract ideas, but complexity needs bounded extraction only | route to Architecture contract refinement + optional Forge utility follow-up | Architecture (+ Forge follow-up) | experiment | medium-high | high | medium | medium | yes |
